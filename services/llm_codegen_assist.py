...
# )
# SLOT_RE = re.compile(
#     r"(?m)^\s*#\s*(?:\{\%\s*raw\s*\%\})?\s*\{\{CUSTOM_BLOCK:([A-Za-z0-9_]+)\}\}\s*(?:\{\%\s*endraw\s*\%\})?\s*$"
# )

SLOT_RE = re.compile(
    r"(?m)^([ \t]*)#\s*(?:\{\%\s*raw\s*\%\}\s*)?\{\{CUSTOM_BLOCK:([A-Za-z0-9_]+)\}\}"
    r"(?:\s*\{\%\s*endraw\s*\%\})?(?:[ \t]*#.*)?$"
)
...
def apply_llm_slots(text: str, slot_payloads: dict) -> str:
    ...
    def _repl(m):
        ...
        payload = (
            slot_payloads.get(slot)
            or slot_payloads.get(slot.lower())
            or slot_payloads.get(slot.upper())
            or ""
        )
