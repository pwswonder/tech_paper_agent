...
#     r"(?m)^(?P<indent>[ \t]*)#\s*\{\{CUSTOM_BLOCK:(?P<name>[a-zA-Z0-9_\-]+)\}\}(?P<trail>[^\n]*)?$"
# )
# _SLOT_LINE_RE = re.compile(
#     r"(?m)^(?P<indent>[ \t]*)#\s*(?:\{\%\s*raw\s*\%\})?\s*\{\{CUSTOM_BLOCK:(?P<name>[A-Za-z0-9_\-]+)\}\}\s*(?:\{\%\s*endraw\s*\%\})?(?P<trail>[^\n]*)?$"
# )
_SLOT_LINE_RE = re.compile(
    r"(?m)^(?P<indent>[ \t]*)#\s*(?:\{\%\s*raw\s*\%\}\s*)?\{\{CUSTOM_BLOCK:(?P<name>[A-Za-z0-9_\-]+)\}\}"
    r"(?:\s*\{\%\s*endraw\s*\%\})?(?P<trail>[^\n]*)?$"
)
...
def render_model_source(
    ...
    # 금지된 Jinja 구문 검사
    txt = open(template_path, "r", encoding="utf-8").read()
    ...
    # 3) ★ 렌더 결과에 custom_blocks 주입 (중요)
    try:
        injected = _postprocess_rendered(
            py_source,
            {"spec": spec, "template_key": template_key},
        )
    except Exception:
        injected = py_source
...
def _is_meaningful_block(code: str) -> bool:
    """
    주입해도 모델 구조를 해치지 않을 '의미있는' 코드인지 빠르게 판정.
    - 비어있거나 공백/주석 뿐이면 False
    - 맨 앞 '# TODO' 이면 False
    - 'raise NotImplementedError' 포함이면 False
    - 'pass'만 있는 경우 False
    - 그 외: 어느 정도 의미 있는 토큰(함수/호출/연산자 등) 포함 여부로 휴리스틱 판정
    """
    ...
    words = __re.findall(r"[A-Za-z_]{4,}", body)
    return len(words) >= 3
...
def _inject_custom_blocks(rendered_text: str, custom_blocks: dict) -> str:
    """
    SAFE 주입 버전:
    - 코드가 '의미있지 않으면' 주입하지 않고 원래 마커를 유지하여 기본 스켈레톤 보존
    - 들여쓰기 0이면 다음 코드 라인의 들여쓰기를 추정하여 적용
    - 주입 결과는 항상 개행으로 종료
    """
    ...
    cb = custom_blocks or {}
    src_snapshot = rendered_text  # 들여쓰기 추정용

    def _infer_next_indent(start_idx: int) -> str:
        tail = src_snapshot[start_idx:]
    ...
    def _repl(m: "_re.Match[str]") -> str:
        ...
        # SAFE: 의미없는 코드는 '주입하지 않음' → 기본 템플릿 흐름 보존
        ...
        for ln in str(code).splitlines():
            # 빈 줄은 원형 보존
            lines.append((indent + ln) if ln.strip() else ln)
        return "\n".join(lines) + "\n"
...
def _postprocess_rendered(_text: str, _locals: dict):
    ...
    cb = (sp2 or {}).get("custom_blocks", {}) or {}
    # ➋ spec['blocks'] → custom_blocks로 병합 (케이스 보강: 원래/upper/lower 모두 키로 주입)
    blocks = (sp2 or {}).get("blocks", {}) or (sp or {}).get("blocks", {})
    if isinstance(blocks, dict):
        for _k, _v in blocks.items():
            code = (_v or {}).get("code") if isinstance(_v, dict) else _v
            if not code:
                continue
            # 원래 키 우선
            cb.setdefault(_k, code)
            # 대소문자 변형도 함께 등록 (템플릿 슬롯명과 스펙 키 케이스 불일치 보호)
            cb.setdefault(_k.upper(), code)
            cb.setdefault(_k.lower(), code)
